...
//#include "Offer.pb.h"
///#include <File.pb.h>
#include "../cmake-build-debug/zhangyixin/proto/File.pb.h"
...
//     install("stop", &MyProcess::stop);
        ...
        install<File>(&Master::report_from_client, &File::process_id,&File::filevalue);
    }

    void report_from_client(const string &key,const string& value) {
        cout << "entering into report" << endl;
        String2Image(value,"./HelloWorld_haha.jar");

//        Try<ProcessTree> res = Fork(None(),Exec("./hello_world &"))();
        Try<ProcessTree> res = Fork(None(),Exec("java -jar ./HelloWorld_haha.jar"))();
...
/*        Offer server_offer;
        ...
        File s_file;
        s_file.set_process_id("serverhahahhaha");
        s_file.set_filevalue("server file value");
        send(clientUPID, s_file);
    }

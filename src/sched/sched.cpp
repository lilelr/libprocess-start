...
protected:
      ...
      const vector<string>& pids)
  {
    ...
    // lilele
     vector<Offer> temp_offers = offers;
      for(size_t i=0;i<offers.size();i++){
      LOG(INFO) << "lilele offers "  << offers[i].id();
          std::cout<<"lilele std "<<" offers "<< offers[i].id()<<std::endl;

          Attribute* t_a = temp_offers[i].add_attributes();
          t_a->set_name("cpu-arch");
         Value_Text* t_value_text = new Value_Text();
         t_value_text->set_value("arm");
          t_a->set_allocated_text(t_value_text);
//         temp_offers[i].set_lele_label("lele_label->OS-kernel:linux-4.0");
//               LOG(INFO) << "lilele label" <<  temp_offers[i].lele_label();
//
//          std::cout<<"lilele "<<" label "<< temp_offers[i].lele_label()<<std::endl;

      }
   const vector<Offer> res_offers = temp_offers;

...
public:
                              ...
                              const vector<Offer>& offers)
  {
    foreach (const Offer& offer, offers) {
      cout << "Received lele offer " << offer.id() << " with " << offer.resources()
           << endl;
//          cout<<" lele label:"<<offer.lele_label()<<endl;
      int attri_size = offer.attributes().size();
        cout<<" lele attri_size "<<attri_size<<endl;
      for(int i=0;i<attri_size;i++){
        Attribute t_a = offer.attributes(i);
        cout<<"Offer id "<<offer.id()<< " Offer attribute"<<i<<"Offer attribute name:  "<<t_a.name()<<"  value_text: " <<t_a.text()<<endl;
      }
